...
function CreateDOMElements(mode){
    ...
    switch(mode){
        ...
        case "journal":
            ...
            iframe.setAttribute("class","iframeC");
            iframe.setAttribute("name","richTextField");
            iframe.contentDocument.body.contentEditable = "true";
            iframe.contentDocument.body.innerHTML = "You need to give a name to your entry to save it. If you want to retrieve a saved entry then type the name of the entry in the text box. Click on any of the buttons to format the text";
            iframe.width = 750;
            iframe.height = 300;
            ...
            let icons = [createElement('i'),createElement('i'),createElement('i'),createElement('i'),createElement('i'),createElement('i'),createElement('i'),createElement('i'),createElement('i')];
            mgBtn2.class('button');
            plBtn1.class('button');
            saveButton.class('button');
            nameInput.class('inpClass');
            icons[0].class('fas fa-bold');
            icons[1].class('fas fa-italic');
            icons[3].class('fas fa-align-center');
            icons[5].class('fas fa-align-justify');
            icons[2].class('fas fa-align-left');
            icons[4].class('fas fa-align-right');
            icons[6].class('fas fa-undo');
            icons[7].class('fas fa-redo');
            icons[8].class('fas fa-underline');
            mgBtn2.position(50,25);
            plBtn1.position(630,25);
            saveButton.position(725,475);
            nameInput.position(40,175);
            icons[0].position(50,205);
            icons[1].position(75,205);
            icons[8].position(100,205);
            icons[2].position(125,205);
            icons[3].position(150,205);
            icons[4].position(175,205);
            icons[5].position(200,205);
            icons[6].position(225,205);
            icons[7].position(250,205);
            mgBtn2.mouseClicked(ChangeModeToMiniGame);
            plBtn1.mouseClicked(ChangeModeToPlay);
            icons[0].mouseClicked(execCmdBold);
            icons[1].mouseClicked(execCmdItalics);
            icons[3].mouseClicked(execCmdAlignCenter);
            icons[5].mouseClicked(execCmdAlignJustify);
            icons[2].mouseClicked(execCmdAlignLeft);
            icons[4].mouseClicked(execCmdAlignRight);
            icons[6].mouseClicked(execCmdUndo);
            icons[7].mouseClicked(execCmdRedo);
            icons[8].mouseClicked(execCmdUnderline);
            saveButton.mouseClicked(saveEntry);
            nameInput.input(searchKeysForEntry);
            nameInput.style('width','20%');
            elements.push(mgBtn2,plBtn1,nameInput,saveButton,iframe,icons[0],icons[1],icons[2],icons[3],icons[4],icons[5],icons[6],icons[7]);
            break;
        case "MiniGame":
